...
import { buildIntakeJobInput, runIntakeExtraction } from './intake';
import {
...
describe('buildIntakeJobInput', () => {
  ...
  it('reads a PDF as a native document block so visual selections survive', () => {
    const richText = 'x'.repeat(500); // a form with a good text layer too
    const input = buildIntakeJobInput(
      {
        mimeType: 'application/pdf',
    ...
    const blocks = input as { type: string; source?: { media_type: string } }[];
    // The PDF itself is sent, and the extracted text rides alongside it.
    expect(blocks[0]!.type).toBe('document');
    expect(blocks[0]!.source!.media_type).toBe('application/pdf');
    expect(blocks.some((b) => b.type === 'text')).toBe(true);
  });

  it('detects a PDF by extension even when the mime type is wrong', () => {
    const input = buildIntakeJobInput(
      {
      ...
      opts(''), // extraction produced nothing
    );
    const blocks = input as { type: string }[];
    expect(blocks[0]!.type).toBe('document');
    // No usable text → PDF only, no empty text block.
    expect(blocks.some((b) => b.type === 'text')).toBe(false);
  });

  it('reads non-PDF formats as plain text', () => {
    const input = buildIntakeJobInput(
      {
        mimeType: 'text/plain',
        filename: 'enquiry.txt',
